/// Non-virtual base reloc via function argument with remaining cleanup.
/// sink(reloc d.base<B1>) followed by more code — verifies B1's base cleanup
/// is deactivated, but other subobject cleanups (B2, own members) still fire.
...
struct B1 : public snoop {
    ...
    B1(B1&& rhs) : snoop(std::move(rhs)), b1x(rhs.b1x) { std::cout << "B1(B1&&) " << this << " <- " << &rhs << std::endl; }
...
struct B2 : public snoop {
    ...
    B2(B2&& rhs) : snoop(std::move(rhs)), b2x(rhs.b2x) { std::cout << "B2(B2&&) " << this << " <- " << &rhs << std::endl; }
...
// D() 0x1
// main ---
// B1Base(B1Base&&) 0x3 <- 0x1
// B1(B1&&) 0x3 <- 0x1
// B2Base(B2Base&&) 0x4 <- 0x2
// B2(B2&&) 0x4 <- 0x2
// D(D&&) 0x3 <- 0x1
// decomp ---
// sink_b1
